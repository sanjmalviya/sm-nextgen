...
export default async function SingleBlogPage({ params }) {
  ...
  // 🔥 QUERY UPDATED: Ab ye content ke andar chhupi hui images ka URL bhi nikalega
  ...
  const displayFormHeading = blog.leadFormHeading || "Quick Consultation 📞";
  const displayFormText = blog.leadFormText || "Drop your details below for a free digital growth strategy call.";

  // 🔥 STRICT TYPOGRAPHY & CUSTOM IMAGE RENDERER
  const customComponents = {
    // Ye block aapki content ke andar daali gayi photos ko design dega
    types: {
      image: ({ value }) => {
    ...
    block: {
      // H1 Size Reduced (Slightly smaller, more elegant)
      ...
      // H2 Size Reduced (text-2xl md:text-3xl)
      ...
      blockquote: ({children}) => (
        <div className="relative my-10 px-8 py-6 bg-gradient-to-r from-[#0097B2]/10 to-transparent border-l-4 border-[#0097B2] rounded-r-2xl">
          <p className="text-xl italic font-medium text-[#0B2545] dark:text-[#E6EEF2]">{children}</p>
        </div>
      ),
    },
    list: {
    ...
    listItem: {
      bullet: ({children}) => (
        <li className="flex items-start gap-3 text-[17px] text-gray-600 dark:text-gray-300">
          <i className="fas fa-check-circle text-[#0097B2] mt-1.5 text-base min-w-[16px]"></i>
          <span>{children}</span>
        </li>
      ),
    },
          ...
          <div className="lg:col-span-4 hidden lg:block relative">
            <div className="sticky top-28 space-y-8">
                ...
                <div className="bg-white dark:bg-[#162032] p-6 md:p-8 rounded-[2rem] shadow-lg border border-gray-100 dark:border-white/5 relative overflow-hidden">
                  ...
                  <div className="space-y-1 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                    {manualToc.map((item, i) => (
                      <a 
                        key={i} 
                        href={`#${slugify(item.headingText)}`} 
                        className="block py-2 px-3 rounded-lg transition-all text-[15px] font-medium border-l-2 border-transparent text-gray-700 dark:text-gray-300 hover:border-[#0097B2] hover:text-[#0097B2] hover:bg-gray-50 dark:hover:bg-white/5"
                      >
                        {item.headingText}
                      </a>
                    ))}
              ...
              <div className="bg-gradient-to-br from-[#0B2545] to-[#11325B] p-8 rounded-[2rem] text-white shadow-xl relative overflow-hidden">
                ...
                <h3 className="text-xl font-bold mb-3 relative z-10">{displayFormHeading}</h3>
                <p className="text-sm text-gray-300 mb-6 relative z-10">{displayFormText}</p>
                  ...
                  {displayServices.map((service, i) => (
                    <li key={i}>
                      <Link href={service.link || "#"} className="flex items-center gap-4 group p-2 hover:bg-gray-50 dark:hover:bg-white/5 rounded-xl transition">
                        <div className="w-12 h-12 rounded-xl bg-[#0097B2]/10 text-[#0097B2] flex items-center justify-center group-hover:bg-[#0097B2] group-hover:text-white transition shadow-sm">
                          <i className={`${service.icon || "fas fa-rocket"} text-lg`}></i>
                        </div>
                        <div>
